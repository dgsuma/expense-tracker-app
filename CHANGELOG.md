...
### Added
- Phase 11: production hardening — Redis-backed rate limiting (per-IP, stricter auth buckets, fail-open), security headers middleware, backup CronJob (daily pg_dump with retention), billing service boundary (no-op free tier), Redis Deployment in K8s, production-hardening doc with security audit checklist.
