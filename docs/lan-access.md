...
Example with PC IP `192.168.1.5`: browse to `http://192.168.1.5:8080` on the phone.
...
2. Find the PC's LAN IP:
   ...
   ```
   Use the Wi-Fi adapter address (e.g. `192.168.1.5`).

## One-time setup

### 1. Rebuild the web image with the LAN API URL

The Flutter web build bakes `API_BASE_URL` at **build time**. For the phone's
browser to reach the API, it must call the PC's LAN IP, not `localhost`
(`localhost` on the phone = the phone itself).

```powershell
$env:WEB_API_BASE_URL = "http://192.168.1.5:8000"
docker compose up -d --build web
```

### 2. Allow the phone's origin in CORS

Add `http://192.168.1.5:8080` to `API_CORS_ORIGINS` in `.env`:

```dotenv
API_CORS_ORIGINS=http://localhost:8080,http://localhost:5000,http://192.168.1.5:8080
```
...
```powershell
curl http://192.168.1.5:8080/            # web app -> HTTP 200
curl http://192.168.1.5:8000/health      # API health -> HTTP 200
```

CORS preflight from the phone's origin should return the allow headers:

```powershell
curl -X OPTIONS http://192.168.1.5:8000/api/v1/auth/login `
  -H "Origin: http://192.168.1.5:8080" `
  -H "Access-Control-Request-Method: POST" `
  -H "Access-Control-Request-Headers: content-type" -D -
# expect: access-control-allow-origin: http://192.168.1.5:8080
```

Then open `http://192.168.1.5:8080` on the phone and log in.

## Troubleshooting

| Symptom | Likely cause | Fix |
|---------|--------------|-----|
| `localhost:8080` works, `192.168.1.5:8080` times out | WSL2 mirrored mode | Switch to NAT mode (above) |
...
| Web loads but API calls fail (CORS) | Origin not allowed | Add phone origin to `API_CORS_ORIGINS` (step 2) |
| Web loads but API calls hit `localhost` | Stale build-time URL | Rebuild web with `WEB_API_BASE_URL` (step 1) |
| `/docs` 404 / "no routes for location" | Wrong port | Swagger is on `:8000/docs`, not `:8080` |

## Caveats

- **DHCP:** if the PC's IP changes, the phone URL breaks and the web image must
  be rebuilt (the API URL is baked in). Set a DHCP reservation / static IP for
  the PC in your router to keep it stable.
...
- **Native Android app:** for an installed app rather than the browser, build
  the APK with the LAN API URL:
  ```powershell
  flutter build apk --dart-define=API_BASE_URL=http://192.168.1.5:8000
  ```
